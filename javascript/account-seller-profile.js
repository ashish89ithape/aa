$(function() {
	...
	$("#sellerinfo_avatar_files, #sellerinfo_banner_files,#sellerinfo_certificate_files").delegate(".ms-remove", "click", function() {
		$(this).parent().remove();
	});	
...
//--------- Uploade certificate------------------//
 var certificateUploader = new plupload.Uploader({
		runtimes : 'gears,html5,flash,silverlight',
		//runtimes : 'flash',
		multi_selection:false,
		browse_button: 'ms-file-sellercertificate',
		url: $('base').attr('href') + 'index.php?route=seller/account-profile/jxUploadSellerAvatar',
		flash_swf_url: 'catalog/view/javascript/plupload/plupload.flash.swf',
		silverlight_xap_url : 'catalog/view/javascript/plupload/plupload.silverlight.xap',
		...
		init : {
			FilesAdded: function(up, files) {
				$('#error_sellerinfo_certificate').html('');
				up.start();
			},
			...
			FileUploaded: function(up, file, info) {
				...
				if (!$.isEmptyObject(data.errors)) {
					...
					$('#error_sellerinfo_certificate').append(errorText).hide().fadeIn(2000);
				}

				if (!$.isEmptyObject(data.files)) {
					for (var i = 0; i < data.files.length; i++) {
						$("#sellerinfo_certificate_files").html(
						'<div class="ms-image">' +
						'<input type="hidden" value="'+data.files[i].name+'" name="seller[licensecertificate]" />' +
						'<img src="'+data.files[i].thumb+'" />' +
						'<span class="ms-remove"></span>' +
						'</div>').children(':last').hide().fadeIn(2000);
					}
			...
			Error: function(up, args) {
				$('#error_sellerinfo_certificate').append(msGlobals.uploadError).hide().fadeIn(2000);
				console.log('[error] ', args);
			}
		}
	}).init();
